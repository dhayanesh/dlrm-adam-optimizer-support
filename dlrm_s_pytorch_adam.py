...
#adam and sparse adam
from optim.custom_adam import Adam
from optim.custom_sparse_adam import SparseAdam

# quotient-remainder trick
from tricks.qr_embedding_bag import QREmbeddingBag
timings = {"scatter": [], "gather": [], "parallel_apply": []}
...
class DLRM_Net(nn.Module):
    ...
    def __init__(
        ...
        if (
            ...
            # save arguments
            self.total_parallel_forward_time = 0.0
...
def run():
    ...
    parser.add_argument("--snapshot-period", type=int, default=1)
    ...
    if not args.inference_only:
        ...
        for name, param in dlrm.named_parameters():
            if 'emb_l' in name:  
                sparse_parameters.append(param)
            else:
                dense_parameters.append(param)
            
        print(f"sparse parameters len: {len(sparse_parameters)}")
        print(f"dense parameters len: {len(dense_parameters)}")

        if args.optimizer == "adam":
            adam_optimizer = Adam(dense_parameters, lr=args.learning_rate)
            sparse_adam_optimizer = SparseAdam(sparse_parameters, lr=args.learning_rate)
            
            optimizer = [adam_optimizer, sparse_adam_optimizer]
        ...
        if args.optimizer == "adam":
            adam_lr_scheduler = LRPolicyScheduler(
                adam_optimizer,
            ...
            sparse_adam_lr_scheduler = LRPolicyScheduler(
    ...
    if args.load_model_default != "":
        ...
        if use_gpu:
            if dlrm.ndevices > 1:
                ...
                # load the model as is on CPU or GPU, with the move
                # to multiple GPUs to be done in parallel_forward
                ld_model = torch.load(f"{args.load_model_default}_rank{ext_dist.my_rank}.pt")
            else:
                # NOTE: when targeting inference on single GPU,
                # note that the call to .to(device) has already happened
                ld_model = torch.load(
                    f"{args.load_model_default}_rank{ext_dist.my_rank}.pt",
                    map_location=torch.device("cuda")
                    # map_location=lambda storage, loc: storage.cuda(0)
                )
        else:
        ...
        print("Load time for default  : {}".format(load_time_default_end - load_time_default_start))
    ...
    with torch.autograd.profiler.profile(
        ...
        if not args.inference_only:
            ...
            while k < args.nepochs:
                ...
                for j, inputBatch in enumerate(train_ld):
                    ...
                    with record_function("DLRM backward"):
                        ...
                        if (
                            ...
                            sparse_adam_optimizer.zero_grad()
                        ...
                        if (
                            ...
                            adam_optimizer.step()
                            sparse_adam_optimizer.step()

                            end_time = time.time()
                            total_opt_time = total_opt_time + (end_time - start_time)
                            
                            #lr_scheduler.step()
                            adam_lr_scheduler.step()
                            sparse_adam_lr_scheduler.step()
                    ...
                    itr_total_time = itr_total_time + (itr_end_time - itr_start_time)
                    print(f"Batch took {itr_end_time - itr_start_time} seconds.")

                loop_end_time = time.time()
                print(f"Batch totally took {itr_total_time} seconds.")
                print(f"Epoch took {loop_end_time - loop_start_time} seconds.")
                ...
                if (args.save_model_default != ""):
                    if (k + 1) % interval == 0:
                        ...
                        #save_path = f"{args.save_model_default}_{k}.pt"
                        save_path = f"{args.save_model_default}_rank{ext_dist.my_rank}.pt"
                        torch.save(model_metrics_dict, save_path)

                        '''ext_dist.barrier()
                        if ext_dist.my_rank == 0:
                            print(f"*** Process {ext_dist.my_rank} - Preparing save model ***")
                            
                            save_path = f"{args.save_model_default}_rank_{ext_dist.my_rank}.pt"
                            torch.save(model_metrics_dict, save_path)
                            
                            print(f"*** Process {ext_dist.my_rank} - Model saved to {save_path} ***")
                        ...
                        total_default_save_time += end_time_default - start_time_default
                        print("Save time for default save : {}".format(end_time_default - start_time_default))
                        print("Total save time for default save : {}".format(total_default_save_time))
            ...
            print("Total time opt:", total_opt_time)
